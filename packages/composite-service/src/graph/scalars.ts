import type { GraphCustomScalarType } from '@composedb/graph-codecs'
import { scalars as graphQLScalars } from '@composedb/graphql-scalars'
import type { JSONSchema, ScalarSchema } from '@composedb/json-schema-codecs'
...
export const SCALAR_RUNTIME_TYPES: Record<ScalarTitle, GraphCustomScalarType> = {
...
const SCALAR_SCHEMA_TITLES = Object.entries(SCALAR_RUNTIME_TYPES).reduce((acc, [title, type]) => {
  acc[type] = title
  return acc
}, {} as Record<string, string>) as Record<GraphCustomScalarType, ScalarTitle>

export function getGraphQLScalarSchema<T extends ScalarSchema = ScalarSchema>(
  type: GraphCustomScalarType
): T & { title: ScalarTitle } {
