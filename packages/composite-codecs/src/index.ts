...
export const ViewFieldDefinitionCodec = io.intersection(
  [FieldCommonDefinitionCodec, GraphViewFieldCodec],
  'ViewFieldDefinition'
)
...
export const LoadModelActionDefinitionCodec = io.strict(
  { action: io.literal('load'), id: io.string, views: ViewsDefinitionCodec },
  'LoadModelDefinition'
)
...
export const GraphQLQueryCodec = io.intersection(
  [
    ...
    io.partial({
      commit: SignedCommitContainerCodec,
      variables: io.UnknownRecord,
      viewerID: didCodec,
    }),
  ],
  'GraphQLQuery'
