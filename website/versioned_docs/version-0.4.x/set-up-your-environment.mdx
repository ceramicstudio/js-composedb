...
The easiest and recommended way to configure your local development environment is by using Wheel - a CLI starter tool for Ceramic that makes it easy to install necessary dependencies
...
for your project. To download the wheel, open your terminal and run the command below:
...
and press Enter on your keyboard for each step. This will install the Ceramic and ComposeDB dependencies and spin up a local node running in-memory.
...
**Important:** Ceramic Anchor Service (CAS) is used to anchor Ceramic streams on a blockchain. Since InMemory option doesn’t use CAS, data generated
for your project will not be persisted. 
