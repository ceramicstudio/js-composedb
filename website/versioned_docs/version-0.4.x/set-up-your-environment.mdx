...
The Ceramic node configuration file will be created inside of the automatically created directory `./ceramic` which can be access using the following command:
...
Apen the `daemon.config.json` file using your preferred code editor and provide the authenticated DID, generated in the [generate your account](#generate-your-account) step of this guide, in the `admin-dids` section of the file as shown below:
...
and spinning the node once again by following the steps in the [Confirmation](#confirmation) step of this guide.
