...
Save this SDL to a file, such as `ball.graphql`. You can then add the model to your node with

    composedb composite:create --output ball.json ball.graphql

From this, you can extract the item's ID

    cat ball.json | js '.models | keys_unsorted[0]'

Next create the SDL for your container
...
Save this SDL to a file, such as `world.graphql`. You can then add the model to your node with

    composedb composite:create --output world.json world.graphql

From this, you can extract the container's ID

    cat world.json | js '.models | keys_unsorted[0]'

Finally, define the relationship between your container and the items it contains

```graphql
type Ball @loadModel(id: "kjzl6hvfrbw6c7dye5scv2pi60rh2qh1wyweld0fhqb41vq51ac8emih9ick5tm") {
  id: ID!
}

type World @loadModel(id: "kjzl6hvfrbw6caa6awyij33sucvc12nfxqcx4454yzmrpe2sq46j29axyqyvohq") {
  balls: [Ball] @relationFrom(model: "Ball", property: "worldID")
}
```

Save this to a file, such as `world_relation.graphql`.
:::caution
This is a view on top of the models, so you cannot require your items, such as with `balls: [Ball!]`

You can create your relation with

    composedb composite:create --output world_relation.json world_relation.graphql

We can now merge all of these and deploy them as a composite.

    composedb composite:merge ball.json world.json world_relation.json --output=merged.json
