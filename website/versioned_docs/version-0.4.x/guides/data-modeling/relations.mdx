...
```
and finally, relate our container to our items
```graphql
type BallRelation @loadModel(id: "kjzl6hvfrbw6caa6awyij33sucvc12nfxqcx4454yzmrpe2sq46j29axyqyvohq") {
  id: ID!
}
...
```
Finally, we can query for the container for the items
```graphql
query {
  worldIndex(first: 1) {
    edges {
