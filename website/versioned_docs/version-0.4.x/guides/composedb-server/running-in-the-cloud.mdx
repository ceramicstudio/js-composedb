...
First, you will have to create your DigitalOcean Kubernetes cluster. To do that, follow an official DigitalOcean tutorial. The process of setting up your Kubernetes cluster will take about 10 minutes. 
Once it’s up and running, you are good to continue with the next step.

:::note

When it comes to choosing your cluster capacity, we recommend starting with the smallest cluster size and upgrading later. For example, to follow this guide you can start with a 1GB RAM and 1vCPU cluster.
...
In this section we will focus on deploying the ComposeDB Server on the DigitalOcean Kubernetes cluster.

1. Clone the [ceramic-infra-scripts repo](https://github.com/3box/ceramic-infra-scripts.git) and enter the created directory.
...
You can use local port forwarding to access the Ceramic node from your local machine. Open a new terminal and run the command below. The result of this command will be the output
showing the port forwarding as shown below. The port forward will stop when the command is exited.
...
The example deployment creates a random seed for the admin DID.
The seed is stored in a Kubernetes secret, which you can view with the following command:



```bash
$ kubectl get secrets  ceramic-admin -o yaml
apiVersion: v1
data:
  private-key: ZjlkZjU3ZjBjODBmZjRiMWU5NGE1NzllMGZhNDgwNGE3M2Y4YmRmOWVlMTY0MjA4YTRiM2JkNmEyZjBkZjM1Nw==
kind: Secret
...
```
...
The `private-key` value is the base64 encoded seed.
