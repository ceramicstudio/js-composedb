...
```sh
npx @ceramicnetwork/cli daemon --network=inmemory
```
...
```sh
pnpm dlx @ceramicnetwork/cli daemon --network=inmemory
```
...
```sh
yarn dlx @ceramicnetwork/cli daemon --network=inmemory
```
...
```sh
composedb composite:from-model kjzl6hvfrbw6ca7nidsnrv78x7r4xt0xki71nvwe4j5a3s9wgou8yu3aj8cz38e --network=testnet-clay --output=my-first-composite.json
```
...
Make sure you have the listed packages installed before running the code below.
...
Running this command requires an [admin DID](./configuration.md#admin-dids) private key to be
