...
:::caution Private key required if you want to run mutations

Running this command in authenticated mode requires an [admin DID](./configuration.mdx#admin-dids) private key to be
provided, using the `DID_PRIVATE_KEY` environment variable or the `--did-private-key` flag.

You will only be able to perform mutations against this local GraphQL server, if you run in in authenticated mode.
