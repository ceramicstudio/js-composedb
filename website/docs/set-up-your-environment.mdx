...
**Important:** [Ceramic Anchor Service (CAS)](https://ceramicnetwork.github.io/docs-docusaurus/docs/composedb/guides/composedb-server/access-mainnet) is used to anchor Ceramic streams on a blockchain.
...
If you are ready to dive into a more advanced configuration, head to [**Wheel reference page→**](https://ceramicnetwork.github.io/docs-docusaurus/docs/wheel/wheel-reference) to learn more details about each parameter you can configure.
...
**Node.js** We've seen best results with Node.js v20. If you are using a different version (e.g. v21+), you may run into issues. Please use `nvm` to install Node.js v20 for best results.

:::

#### Ceramic

ComposeDB runs on Ceramic v3.x, so you will need to run a Ceramic node. To get started, we recommend running a local Ceramic node. If you're interested in running the production node, you can follow the [tutorial here](https://blog.ceramic.network/configuring-a-ceramic-node-using-aws/).
...
```bash
npm install --location=global @ceramicnetwork/cli@next
```
...
```bash
yarn global add @ceramicnetwork/cli@next
```
