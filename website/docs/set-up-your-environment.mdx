...
Not sure which versions you have? Use this command to make sure you have the correct versions installed.
```jsx
node -v
pnpm -v
```
...
1. [@composedb/devtools](https://composedb.js.org/docs/0.2.x/api/modules/devtools) containing utilities related to managing composites, and 
2. [@composedb/devtools-node](https://composedb.js.org/docs/0.2.x/api/modules/devtools_node) which contains utilities for interacting with the local file system and starting a local HTTP server. 
...
## Setup
### Run a Ceramic node
You can check that everything was installed correctly by spinning up the Ceramic node. Running the command below will start the Ceramic node in local mode and connect to Clay testnet. 
...
>***Note:*** After you start a node, a node configuration file is created. Later in this guide, you’ll be editing this file. You can find it at the following path: `~/.ceramic/daemon.config.json`
...
## Next steps
After setting up your environment, the next step is to **[Create your composite](./create-your-composite) →**
