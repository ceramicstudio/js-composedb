...
In order to perform mutations, the Ceramic instance used by the Compose client needs to be authenticated. This can be done by calling the `setDID` method on the `ComposeClient` instance with an authenticated `DID` instance:
