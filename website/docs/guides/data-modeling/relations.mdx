...
## Model to Account
Model to account relations enable you to link data to and query data from an account other than the data’s author. When using this type of relation, you need to define a model field that stores an account (e.g. a [DID](../../guides/composedb-client/user-sessions)), then add the `@accountReference` directive to make it queryable.
...
Head to [**Composites**](../data-modeling/composites) next to learn more [**→**](../data-modeling/composites)