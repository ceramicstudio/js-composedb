...
Sorting allows you to sort the returned documents by a specified order - `ascending` or `descending`. For a field to be used for sorting the results, it needs to have an index created just like we specified in [Create Indices](#create-indices) section of this guide.
