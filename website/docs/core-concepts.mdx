...
ComposeDB provides a graph structure for interacting with data on Ceramic, with two types of nodes [accounts](./graph-structure#accounts) & [documents](./graph-structure#documents), and edges between nodes representing relations. [**→**](./graph-structure.mdx)
